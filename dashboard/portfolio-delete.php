...
$status_select = "SELECT * FROM portfolio WHERE id = $id";
...
if ($status_select_assoc['trash_status'] == 1) {
    $update = "UPDATE portfolio SET trash_status = 2 WHERE id=$id";
    $update_query = mysqli_query($db_connection, $update);
    header('location:portfolio.php');
}
else {
    $update = "UPDATE portfolio SET trash_status = 1 WHERE id=$id";
    $update_query = mysqli_query($db_connection, $update);
    header('location:portfolio.php');
}
